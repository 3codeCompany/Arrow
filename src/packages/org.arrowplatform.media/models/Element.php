...
class Element extends \Arrow\ORM\ORM_Arrow_Package_Media_Element
{
    ...
    private static function appendFile($folder, $file, $nameSuggestion = "", $delete = false, $realName = false)
    {
        ...
        if ($nameSuggestion)
            $name = \Arrow\Package\Utils\StringHelper::toValidFilesystemName($nameSuggestion, false);
        else
        ...
        $nameWithExtension = \Arrow\Package\Utils\StringHelper::toValidFilesystemName($name );
        $path = $folderPath . "/" . $nameWithExtension;

        $i=1;
        while(file_exists(MediaAPI::getBasePath().$path)){
            $nameWithExtension = \Arrow\Package\Utils\StringHelper::toValidFilesystemName($path_parts['filename'] . $i.".".$path_parts['extension'] );
            $path = $folderPath . "/" . $nameWithExtension;
            $i++;
        }
